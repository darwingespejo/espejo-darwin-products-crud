...
<body class="grid-bg flex min-h-screen items-center justify-center p-4 text-slate-100 sm:p-8">
    <div class="panel w-full max-w-xl overflow-hidden rounded-2xl">
        <div class="border-b border-blue-500/20 bg-slate-950/70 px-6 py-6 sm:px-8">
            <p class="mono mb-2 text-xs font-medium uppercase tracking-[.24em] text-blue-400">Inventory / New record</p>
            <h1 class="text-3xl font-semibold tracking-tight text-white">Add product</h1>
            <p class="mt-1 text-sm text-slate-400">Create a new item in the catalog.</p>
        </div>
        ...
        <?php endif; ?>
        <form action="<?= site_url('products/create'); ?>" method="POST" class="space-y-5">
            <div>
                <label class="mono block text-xs font-medium uppercase tracking-[.14em] text-slate-400">Product name</label>
                <input type="text" name="product_name" required class="field mt-2 w-full rounded-lg px-4 py-3 text-sm placeholder:text-slate-600" placeholder="e.g. Wireless Headphones">
            </div>
            <div>
                <label class="mono block text-xs font-medium uppercase tracking-[.14em] text-slate-400">Description</label>
                <textarea name="description" rows="4" class="field mt-2 w-full resize-none rounded-lg px-4 py-3 text-sm placeholder:text-slate-600" placeholder="Add a short description..."></textarea>
            </div>
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    <label class="mono block text-xs font-medium uppercase tracking-[.14em] text-slate-400">Price</label>
                    <input type="number" step="0.01" name="price" required class="field mt-2 w-full rounded-lg px-4 py-3 text-sm" placeholder="0.00">
                </div>
                <div>
                    <label class="mono block text-xs font-medium uppercase tracking-[.14em] text-slate-400">Quantity</label>
                    <input type="number" name="quantity" required class="field mt-2 w-full rounded-lg px-4 py-3 text-sm" placeholder="0">
                </div>
            </div>
            <div class="flex items-center justify-between border-t border-slate-800 pt-5">
                ...
                <button type="submit" class="rounded-lg bg-blue-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-950/50 transition hover:bg-blue-500">Save product</button>
            </div>
        </form>
        </div>
    </div>
